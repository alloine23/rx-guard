...
import { Loader2, Pill, Plus, Trash2, UserX } from 'lucide-react'
...
export default function AdminPharmacistsPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Pharmacist | null>(null)
  ...
  const fetchPharmacists = useCallback(async () => {
    try {
      ...
      const data: Pharmacist[] = await res.json()
      setPharmacists(data)
    } catch {
      setFetchError('Unable to load pharmacists list.')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDialogOpen(false)
      resetForm()
      sileo.success({ title: 'Pharmacist registered', description: `${email} has been added to your pharmacy.` })
  ...
  const handleDelete = async () => {
    ...
    // Optimistic remove
    setPharmacists((prev) => prev.filter((p) => p.id !== target.id))

    try {
      const res = await fetch(`/api/admin/pharmacists/${target.id}`, {
        method: 'DELETE',
      })
      if (!res.ok) {
        // Revert
        setPharmacists((prev) => [...prev, target].sort(
          (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
        ))
        const body = await res.json().catch(() => ({}))
        sileo.error({ title: 'Failed to delete pharmacist', description: body.error ?? 'Please try again.' })
        return
      }
      sileo.success({ title: 'Pharmacist removed', description: `${target.email} has been deleted.` })
    } catch {
      setPharmacists((prev) => [...prev, target].sort(
        (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      ))
  ...
  const columns: ColumnDef<Pharmacist>[] = [
    {
      accessorKey: 'email',
      header: ({ column }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const isLoading = deleteLoading === pharmacist.id
        return (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger
                render={
                  <Button
                    ...
                    disabled={isLoading}
                    onClick={() => setDeleteTarget(pharmacist)}
                  />
                }
              >
                {isLoading ? (
              ...
              <TooltipContent>Delete pharmacist</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )
      },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">Pharmacists</h2>
          <p className="text-sm text-muted-foreground">
            {pharmacists.length} {pharmacists.length === 1 ? 'pharmacist' : 'pharmacists'} registered
          </p>
        </div>
        <Dialog
          open={dialogOpen}
        ...
        >
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Register New Pharmacist</DialogTitle>
              <DialogDescription>
                Create credentials for a new pharmacist in your pharmacy.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input
                  id="pharmacist-email"
                  type="email"
                  placeholder="pharmacist@pharmacy.ph"
                  required
                  value={email}
      ...
      {pharmacists.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-border/50 bg-card py-16 text-center">
          ...
          <h3 className="mt-4 text-sm font-semibold">No pharmacists registered</h3>
          <p className="mt-1 max-w-sm text-xs text-muted-foreground">
            Get started by registering a pharmacist to your pharmacy using the button above.
          </p>
        </div>
      ) : (
        <DataTable
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null) }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete pharmacist?</AlertDialogTitle>
            <AlertDialogDescription>
